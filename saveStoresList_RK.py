...
def saveStorenames_altRK(inputParametersPath, data, filepath):
    if isinstance(data, pd.DataFrame):
        data['name'] = np.asarray(data['name'], dtype=np.str)
        allnames = np.unique(data['name'])
        index = []
    ...
    if any(x in filename for x in ["_RI30", "_RI60", "_FR1R","_FR1L", "_Shock"]):
        ...
        raise ValueError('Event naming contingencies did not understand this file... see saveStorenames_altRK')
...
#the below replaces execute in that it calls the proper _altRK version of saveStoresName 
#and can replace the normal execute function in the savingInputParameters.ipynb GUI page
#to make prior changes, modify the execute() function and saveStorenames to RK versions, 
    #then modify savingInputParameters.ipynb to call the proper execute RK versions
def execute_autoRK(inputParameters):
    ...
    for i in folderNames:
        ...
        saveStorenames_altRK(inputParametersPath, data, filepath)
        
    print("Finished RK storeList allocation")

inputParametersPath = r"C:\Users\rfkov\Documents\SynapseData\LBN_Synapse_Data\LBN_064_OmiR1-220519-143106\GuPPyParamtersUsed.json"
with open(inputParametersPath) as f:    
     inputParameters = json.load(f)
filepath=os.path.abspath(os.path.join(inputParametersPath, os.pardir))
data = readtsq(filepath)
#saveStorenames(inputParametersPath, data, event_name, flag, filepath)
saveStorenames_altRK(inputParametersPath, data, filepath)
# In[ ]:
